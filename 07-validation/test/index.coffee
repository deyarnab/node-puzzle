...
describe '07-validation', ->
  ...
  it 'should return `false` for invalid data: email', ->
    assert !validate
      id: 1
      name: 'John Doe'
      email: Array(300).join('a') + '@bar.com'
      taxRate: 0.1
      favouriteColour: '#ccccffx' # <--- problem
      interests: ["cycling", "programming"]
  ...
  it 'should return `false` for invalid data: taxRate', ->
    assert !validate
      id: 1
      name: 'John Doe'
      email: Array(200).join('a') + '@bar.com'
      taxRate: 1 # <--- problem
      favouriteColour: '#ff6' 
      interests: ["cycling", "programming"]    

  it 'should return `false` for invalid length of array of : interests and also for long interest string', ->
    assert !validate
      id: 1
      name: 'John Doe'
      email: Array(200).join('a') + '@bar.com'
      taxRate: 0.1 
      favouriteColour: '#ff6' 
      interests: ["cycling", "programming-programming-programming-programming", "hiking", "swimming", "basketball"]  # <--- problem  
